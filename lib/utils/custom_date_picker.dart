...
class CustomCupertinoDatePicker extends StatefulWidget {
  ...
  /// [CustomCupertinoDatePickerMode.date] mode, this callback will always report the
  ...
  CustomCupertinoDatePicker({
    ...
    assert(
      mode != CustomCupertinoDatePickerMode.dateAndTime || minimumDate == null || !this.initialDateTime.isBefore(minimumDate!),
      'initial date is before minimum date',
    );
    assert(
      mode != CustomCupertinoDatePickerMode.dateAndTime || maximumDate == null || !this.initialDateTime.isAfter(maximumDate!),
      'initial date is after maximum date',
    );
    assert(
      mode != CustomCupertinoDatePickerMode.date || (minimumYear >= 1 && this.initialDateTime.year >= minimumYear),
      'initial year is not greater than minimum year, or minimum year is not positive',
    );
    assert(
      mode != CustomCupertinoDatePickerMode.date || maximumYear == null || this.initialDateTime.year <= maximumYear!,
      'initial year is not smaller than maximum year',
    );
    assert(
      mode != CustomCupertinoDatePickerMode.date || minimumDate == null || !minimumDate!.isAfter(this.initialDateTime),
      'initial date ${this.initialDateTime} is not greater than or equal to minimumDate $minimumDate',
    );
    assert(
      mode != CustomCupertinoDatePickerMode.date || maximumDate == null || !maximumDate!.isBefore(this.initialDateTime),
...
class _CupertinoDatePickerDateTimeState extends State<CustomCupertinoDatePicker> {
  ...
  void didChangeDependencies() {
    ...
    alignCenterLeft = textDirectionFactor == 1 ? Alignment.center : Alignment.center;
    alignCenterRight = textDirectionFactor == 1 ? Alignment.center : Alignment.center;
