...
void main() {
  testWidgets('complete profile widget test', (WidgetTester tester) async {
    // Build our app and trigger a frame.
    await tester.pumpWidget(const MaterialApp(home: SetupProfileScreen()));

    // Verify widgets
    expect(find.byKey(const Key("save")), findsOneWidget);
    expect(find.byKey(const Key("Designation")), findsOneWidget);
    expect(find.byKey(const Key("Agency Name")), findsOneWidget);
    expect(find.byKey(const Key("Company name")), findsOneWidget);
    await tester.pump();
  });
}
